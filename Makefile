...
ifneq ($(SANITIZERS),)
	...
	ifeq ($(findstring address,$(SANITIZERS)),address)
		CMAKE_SANITIZER_FLAGS += -DSTDLIBX_USE_SANITIZER_ADDRESS=ON -DSTDLIBX_USE_SANITIZER_LEAK=ON
	endif
	
	ifeq ($(findstring undefined,$(SANITIZERS)),undefined)
		CMAKE_SANITIZER_FLAGS += -DSTDLIBX_USE_SANITIZER_UNDEFINED=ON
	endif
	
	ifeq ($(findstring thread,$(SANITIZERS)),thread)
		CMAKE_SANITIZER_FLAGS += -DSTDLIBX_USE_SANITIZER_THREAD=ON
	endif
	
	ifeq ($(findstring memory,$(SANITIZERS)),memory)
		CMAKE_SANITIZER_FLAGS += -DSTDLIBX_USE_SANITIZER_MEMORY=ON
	endif
	
	ifeq ($(SANITIZERS),all)
		CMAKE_SANITIZER_FLAGS := -DSTDLIBX_USE_SANITIZER_ADDRESS=ON -DSTDLIBX_USE_SANITIZER_UNDEFINED=ON -DSTDLIBX_USE_SANITIZER_LEAK=ON
	endif
endif
