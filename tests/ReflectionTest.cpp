...
void test_base_to_class() {
    ...
    // Mirror::operator Info() applies in the converted constant expression, so
    // the trailing .value() is not needed on the template argument.
    ...
    template for (constexpr Base b: CIRCLE_BASES) {
        constexpr StringView name = Ops::class_of<b.type()>().name().value_or("");
