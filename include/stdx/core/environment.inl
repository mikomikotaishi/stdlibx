...
export namespace stdx::core {
    ...
    public:
         ...
         * @brief An operating system which is reported by Environment.
         */
        ...
        /**
         * @enum Compiler
         * @brief A compielr which is reported by Environment.
         */
        enum class Compiler: u8 {
            UNKNOWN = 0, ///< An unknown or undetectable compiler.
            GCC, ///< GNU Compiler Collection.
            CLANG, ///< LLVM/Clang.
            APPLE_CLANG, ///< Apple LLVM/Clang.
        ...
        [[nodiscard]]
        static constexpr OperatingSystem operating_system() noexcept {
            #ifdef __linux__
        ...
        [[nodiscard]]
        static constexpr Compiler compiler() noexcept {
...
namespace stdx::fmt {
    ...
    template <>
    struct Formatter<Environment::Compiler> {
        enum class Mode: u8 {
            SHORT,
            VERBOSE,
        };

        Mode mode = Mode::SHORT; // Default to short if no specifier is provided
        ...
        constexpr auto parse(FormatParseContext& ctx) {
            ...
            if (it != end && *it != '}') {
                throw FormatException("Invalid specifier"); 
            }
            return it;
        }

        auto format(Environment::Compiler compiler, FormatContext& ctx) const {
            StringView compiler_name;
            bool verbose = (mode == Mode::VERBOSE);

            switch (compiler) {
                case Environment::Compiler::UNKNOWN:
                    compiler_name = "Unknown";
                    break;
                case Environment::Compiler::GCC:
                    compiler_name = verbose ? "GNU Compiler Collection" : "GCC";
                    break;
                case Environment::Compiler::CLANG:
                    compiler_name = verbose ? "LLVM Clang" : "Clang";
                    break;
                case Environment::Compiler::APPLE_CLANG:
                    compiler_name = "Apple Clang";
                    break;
                case Environment::Compiler::MSVC:
                    compiler_name = verbose ? "Microsoft Visual C++" : "MSVC";
                    break;
            }
            return format_to(ctx.out(), "{}", compiler_name);
        }
    };
...
template <>
struct stdx::fmt::formatter<Environment::Compiler>: public Formatter<Environment::Compiler> {};
