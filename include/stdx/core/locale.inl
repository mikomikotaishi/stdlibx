...
export namespace stdx::core {
    ...
    private:
        Self _locale; ///< The wrapped locale (defaults to a copy of the global locale).
    ...
    public:
        ...
        Locale(const Locale& other, const char* name, Category categories) try:
            _locale{Self(other._locale, name, categories)} {
        } catch (...) {
            throw LocaleException(std::format("unsupported locale: {}", name));
        }
        ...
        Locale(const Locale& other, const Locale& one, Category categories) try:
            _locale{Self(other._locale, one._locale, categories)} {
        } catch (...) {
        ...
        Locale(const Locale& other, F* facet) try:
            _locale{Self(other._locale, facet)} {
        } catch (...) {
        ...
        text::TextEncoding encoding() const noexcept {
            const String locale_name = _locale.name();
        ...
        Locale combine(const Locale& other) const {
            return Locale(_locale.combine<F>(other._locale));
        }
        ...
        static Locale global(const Locale& loc) {
            return Locale(Self::global(loc._locale));
        }
         ...
         * an empty result reflects a host that lacks that locale.
