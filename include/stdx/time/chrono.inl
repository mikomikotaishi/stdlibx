...
export namespace stdx::time::chrono {
    ...
    /**
     * @enum DateTextLength
     * @brief Length variant for textual names of date/time fields.
     *
     * Stand-alone variants are not currently distinguished.
     */
    enum class DateTextLength: u8 {
        FULL, ///< "January", "Reiwa",  "Monday"
        SHORT, ///< "Jan", "R", "Mon"
        NARROW, ///< "J", "令和", "M"
    };
    ...
    /**
     * @concept ChronologyTextLike
     * @brief Refines ChronologyLike with textual names for eras and months.
     *
     * Required by DateTimeFormatter pattern letters that produce or
     * consume text (G, MMM+, ...). Returned strings are the chronology's
     * own canonical defaults (English where applicable; native script
     * available via DateTextLength::NARROW for chronologies where that is
     * meaningful, e.g. Japanese era kanji). Localization is the concern
     * of a separate provider layer, not this concept.
     *
     * The year is passed to month_name / parse_month so lunisolar
     * chronologies (Hebrew, Chinese) can disambiguate leap-year months
     * such as Adar I and Adar II. Fixed-12-month chronologies may ignore
     * the year argument.
     */
    template <typename T>
    concept ChronologyTextLike = ChronologyLike<T> &&
        requires (typename T::Era era, i32 y, u32 m, DateTextLength s, StringView sv) {
        { T::era_name(era, s) } noexcept -> ConvertibleTo<StringView>;
        { T::month_name(y, m, s) } -> ConvertibleTo<StringView>;
        { T::parse_era(sv, s) } noexcept -> SameAs<Optional<typename T::Era>>;
        { T::parse_month(sv, y, s) } noexcept -> SameAs<Optional<u32>>;
    };
